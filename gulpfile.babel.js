...
function html() {
  return src(`${localServer.src}**/*.html`)
    ...
    ) //видаляє коментарі при Білд сборці
...
function css() {
  return src(`${localServer.assets}sass/main.scss`)
    ...
    .pipe(
      autoprefixer({ overrideBrowserslist: ["IE 6", "Chrome 9", "Firefox 14"] })
    )
...
function openLocal() {
  return src(`${localServer.out}index.html`).pipe(
    open({ uri: `${localServer.url}${localServer.port}/` })
  );
}

// setTimeout(openLocal, 2000); //Відкриває автоматов в браузері

exports.build = parallel(clean, html, css, js, img); //тут послідовність функцій які запускаются, наприклад ви можете додати щоб автоматом браузер відкривався openLocal
